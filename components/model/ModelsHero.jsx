...
const initialCards = [
  { id: 1, src: "/models/Rectangle 92.png", alt: "Card 1" },
  { id: 2, src: "/models/Rectangle 90.png", alt: "Card 2" },
  {
    id: 3,
    src: "/models/vid.mp4",
    alt: "WAN 2.5",
  ...
  { id: 4, src: "/models/Rectangle 91.png", alt: "Card 4" },
  { id: 5, src: "/models/Rectangle 89.png", alt: "Card 5" },
];

export default function ModelsShowcaseHero() {
  const [cards, setCards] = useState(initialCards);
  const [background, setBackground] = useState(initialCards[2]); 
  ...
  const rotateCards = () => {
    setCards((prev) => {
      ...
      const newMain = newOrder[2]; 
      setBackground(newMain);
      return newOrder;
    });
  };
  ...
  useEffect(() => {
    const mainCard = cards[2];
    clearTimeout(timerRef.current);

    if (mainCard.src.endsWith(".mp4")) {
      const videoEl = videoRef.current;
      if (videoEl) {
        const handleEnd = () => rotateCards();
        videoEl.addEventListener("ended", handleEnd);
        return () => videoEl.removeEventListener("ended", handleEnd);
      }
    } else {
      timerRef.current = setTimeout(rotateCards, 5000);
    }
      ...
      <div className="relative z-10 max-w-7xl w-full flex items-center justify-between">
        ...
        <div className="w-1/2 relative flex justify-center">
          <div className="flex items-center justify-center relative">
            <AnimatePresence>
              {cards.map((card, idx) => {
                const isMain = idx === 2; // center card
                return (
                  <motion.div
                    key={card.id}
                    layout
                    transition={{ type: "spring", stiffness: 150, damping: 20 }}
                    className={`absolute rounded-[26px] overflow-hidden bg-[#111] ${positions[idx]} ${
                      isMain ? "cursor-pointer" : ""
                    }`}
                    style={{
                      width: isMain ? "240px" : "200px",
                      height: isMain ? "380px" : "370px",
                    }}
                  >
                    <AnimatePresence mode="wait">
                      <motion.div
                        key={isMain ? `${card.id}-main` : `${card.id}-thumb`}
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        exit={{ opacity: 0 }}
                    ...
                    {/* ✅ Overlay for Main Video */}
                    {isMain && card.src.endsWith(".mp4") && (
                      <>
